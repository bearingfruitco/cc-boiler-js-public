#!/usr/bin/env python3
"""
Add all v4.0 hooks to Claude settings
"""
...
def update_settings():
    ...
    new_hooks = [
        ...
        {
            "type": "command", 
            "command": "python3 .claude/hooks/pre-tool-use/21-docs-first-enforcer.py"
        },
        {
            "type": "command",
            "command": "python3 .claude/hooks/pre-tool-use/22-api-docs-generator.py"
        },
        {
            "type": "command",
            "command": "python3 .claude/hooks/pre-tool-use/23-a11y-enforcer.py"
        }
    ]
        ...
        if hook_group['matcher'] == "":
            ...
            for new_hook in new_hooks:
                if new_hook['command'] not in existing_commands:
                    hook_group['hooks'].append(new_hook)
                    print(f"✅ Added: {new_hook['command'].split('/')[-1]}")
    
    # Backup current settings
    backup_path = settings_path.with_suffix('.json.backup-v4-complete')
    ...
    print(f"✅ Updated settings.json with all v4.0 hooks")
