...
def main():
    """Main hook logic"""
    try:
        # Read input - now includes session info per official docs
        ...
        if tool_name not in ['Write', 'Edit', 'MultiEdit']:
            # Exit code 0 with no output = continue normally
            sys.exit(0)
        ...
        if violations:
            ...
            if is_intentional:
                # If intentional, just output warning but continue
                ...
                # Output to stdout for transcript mode
                print(warning_msg)
                sys.exit(0)
            
            elif high_severity:
                # Block with exit code 2 - stderr goes to Claude
